...
class HMC5883L {
    ...
    public:
        ...
        std::tuple<float, float, float> readSensor() {
            char buf[6];
            int status = i2cReadI2CBlockData(i2c_handle, 0x03, buf, 6);
            if (status != 6) {
        ...
        void initialize() {
            // Calibrar el sensor
            char buf[3];
            buf[0] = 0x00; // Registro de configuración
            buf[1] = 0x70; // Configuración: 8 muestras, 75 Hz
            buf[2] = 0x00; // Modo: continuo
            int status = i2cWriteI2CBlockData(i2c_handle, 0x00, buf, 3);
            if (status != 0) {
                throw std::runtime_error("Error al escribir en el sensor");
            }
            // establecer r0, p0 y y0 como los valores leídos
            char initBuf[6];
            status = i2cReadI2CBlockData(i2c_handle, 0x03, initBuf, 6);
            if (status != 6) {
